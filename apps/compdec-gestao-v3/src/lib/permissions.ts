...
export const can = (role: Role | undefined, action: string) => {
  ...
  const matrix: Record<Role, string[]> = {
    SUPER_ADMIN: ['*'],
    GESTOR_COMPDEC: ['dashboard','inventory','nupdecs','teams','volunteers','activities','alerts','documents','audit','users','readiness'],
    COORDENADOR: ['dashboard','inventory','nupdecs','teams','volunteers','activities','alerts','documents','audit','readiness'],
